            ...
            <div class="crm-unified-card">
                ...
                <div class="exchange-rate-block">
                    ...
                    <div class="rate-info-grid">
                        @php
                            $latestRate =
                                isset($sales) && $sales->count() > 0
                                    ? $sales->whereNotNull('Exchange_Rate_At_Sale')->sortByDesc('Sale_Date')->first()
                                        ?->Exchange_Rate_At_Sale
                                    : null;
                            $rateDisplay = $latestRate ? number_format($latestRate, 4) . ' PHP/USD' : 'Not set';
                            $lastSaleDate =
                                isset($sales) && $sales->count() > 0
                                    ? $sales->sortByDesc('Sale_Date')->first()?->Sale_Date
                                    : null;
                            $lastUpdated = $lastSaleDate
                                ? \Carbon\Carbon::parse($lastSaleDate)->diffForHumans()
                                : 'Never';
                        @endphp
                        <div class="info-item">
                ...
                <div class="fin-controls">
                    <div class="fin-search-container">
                        <div class="fin-search-wrapper">
                            <svg class="fin-search-icon" width="16" height="16" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2">
                                <circle cx="11" cy="11" r="8"></circle>
                                <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
                            </svg>
                            <input type="text" class="fin-search-input"
                                ...
                                <tr class="sale-row" data-sale-id="{{ $sale->Sale_ID }}"
                                    ...
                                    <td class="table-cell">
                                        <div class="sale-id-text">
                                            {{ $sale->Invoice_Number ?? '#SL-' . str_pad($sale->Sale_ID, 5, '0', STR_PAD_LEFT) }}
                                        </div>
                                    </td>
                                    <td class="table-cell">
                                        <div class="customer-name">{{ $sale->customer->Institution_Name ?? 'N/A' }}
                                        </div>
                                    </td>
                                    <td class="table-cell">
                                    ...
                                    <td class="table-cell">
                                        <div class="date-text">
                                            {{ $sale->Due_Date ? \Carbon\Carbon::parse($sale->Due_Date)->format('M d, Y') : 'N/A' }}
                                        </div>
                                    </td>
                                    <td class="table-cell">
                                    ...
                                    <td class="table-cell">
                                        <div class="table-actions">
                                            <a href="#" class="fin-action-btn view-invoice-btn"
                                                data-sale-id="{{ $sale->Sale_ID }}" title="View Details">
                                            ...
                                            @if ($sale->Status !== 'Completed' && $sale->Status !== 'Paid')
                                                <a href="#" class="fin-action-btn record-payment-btn"
                                                    data-sale-id="{{ $sale->Sale_ID }}"
                                                    data-sale-num="{{ $sale->Invoice_Number ?? 'SL-' . str_pad($sale->Sale_ID, 5, '0', STR_PAD_LEFT) }}"
                                                    data-total="{{ $sale->Total_Amount_PHP }}"
                                                    data-paid="{{ $sale->Amount_Paid ?? 0 }}" title="Record Payment">
                                                    <svg width="16" height="16" viewBox="0 0 24 24"
                                                        fill="none" stroke="currentColor" stroke-width="2">
                                                        <circle cx="12" cy="12" r="10"></circle>
                                            ...
                                            @if (($sale->Amount_Paid ?? 0) == 0 && $sale->Status !== 'Completed')
                                                <a href="#" class="fin-action-btn delete-invoice-btn"
                                                    data-sale-id="{{ $sale->Sale_ID }}"
                                                    data-invoice-num="{{ $sale->Invoice_Number ?? 'SL-' . str_pad($sale->Sale_ID, 5, '0', STR_PAD_LEFT) }}"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function applyFilters() {
            ...
            document.querySelectorAll('.sale-row').forEach(row => {
                ...
                const matchSearch = !searchTerm || invoiceId.includes(searchTerm) || customerName
                    .includes(searchTerm);
        ...
        document.querySelectorAll('.view-invoice-btn').forEach(btn => {
            btn.addEventListener('click', async function(e) {
                ...
                const status = row.querySelector('.status-badge')?.textContent?.trim() ||
                    '';
                const totalAmount = parseFloat(row.querySelector('.record-payment-btn')
                    ?.dataset.total || amount.replace(/[₱,]/g, '')) || 0;
                const amountPaid = parseFloat(row.querySelector('.record-payment-btn')
                    ?.dataset.paid || 0);
            ...
            `;
                createModal('Invoice Details — ' + invoiceNum, body,
                    `<button class="fin-btn secondary fin-close-modal">Close</button>`);
            });
        });
        ...
        document.querySelectorAll('.delete-invoice-btn').forEach(btn => {
            ...
            `);
                ...
                modal.querySelector('#confirmDeleteInvoice').addEventListener('click',
                    async function() {
                        this.disabled = true;
                        ...
                        try {
                            ...
                            if (json.success) {
                                ...
                                showToast(json.message || 'Failed to delete invoice',
                                    'error');
                            }
                        } catch (err) {
    ...
    `;
        ...
        `;
            ...
            modal.querySelector('#submitSale').addEventListener('click', async function() {
                await submitForm(modal, '#saleForm', '{{ route('admin.finance.sale') }}',
        ...
        document.querySelectorAll('.record-payment-btn').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                const saleId = this.dataset.saleId;
                const row = this.closest('.sale-row');
                const saleNum = row?.querySelector('.sale-id-text')?.textContent || 'SL-' +
                    saleId;
            ...
            `;
                ...
                modal.querySelector('#confirmPayment').addEventListener('click',
                    async function() {
                        await submitForm(modal, '#paymentForm',
                            `/admin/finance/sale/${saleId}/payment`,
        ...
        document.querySelector('.payment-plans-btn')?.addEventListener('click', function(e) {
            ...
            if (!cards) cards =
                '<div style="text-align:center;padding:2rem;color:#64748b;">No payment plans found.</div>';
        ...
        `;
            const modal = createModal('Payment Plans', body,
                `<button class="fin-btn secondary fin-close-modal">Close</button>`);
        ...
        `;
            ...
            modal.querySelector('#submitPlan').addEventListener('click', async function() {
                ...
                await submitForm(modal, '#paymentPlanForm',
                    '{{ route('admin.finance.payment-plan') }}',
        ...
        async function openPlanDetailsModal(planId) {
            ...
            `;
                const modal = createModal(`Plan PP-${planId.toString().padStart(3, '0')}`, body,
                    `<button class="fin-btn secondary fin-close-modal">Close</button>`);
        ...
        document.querySelector('.overdue-alerts-btn')?.addEventListener('click', function(e) {
            ...
            if (!cards) cards =
                '<div style="text-align:center;padding:2rem;color:#64748b;">No overdue transactions. 🎉</div>';
        ...
        `;
            const modal = createModal('Overdue Invoice Alerts', body,
                `<div class="close-right-footer"><button class="fin-btn secondary fin-close-modal">Close</button></div>`
            );

            modal.querySelectorAll('.send-reminder-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const saleId = this.dataset.saleId;
                    showToast(`Reminder sent for INV-${saleId.padStart(3, '0')}!`,
                        'success');
                });
            });
        });
        ...
        async function submitForm(modal, formSelector, url, successMsg) {
            ...
            try {
                ...
                if (response.ok && (result.success || result.invoice)) {
                    ...
                    if (errorsDiv) {
                        errorsDiv.innerHTML = result.errors ? Object.values(result.errors).flat().join(
                            '<br>') : (result.message || 'Operation failed.');
                        errorsDiv.style.display = 'block';
                    }
                }
            } catch (err) {
                if (errorsDiv) {
                    errorsDiv.innerHTML = 'Network error.';
                    errorsDiv.style.display = 'block';
                }
            } finally {
...
<style>
    ...
    .fin-modal-overlay.show .fin-modal-content {
