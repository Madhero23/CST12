        ...
        <div class="admin-inventory">
            ...
            <div class="inventory-dashboard-card">
                    ...
                    <div class="dashboard-actions">
                        <button class="btn-outline" type="button" id="openAgingReportBtn">
                            <svg class="btn-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"
                                ...
                                <path
                                    d="M9 12H15M9 16H15M3 7V17C3 19.2091 4.79086 21 7 21H17C19.2091 21 21 19.2091 21 17V7M3 7L12 3L21 7M3 7L12 11L21 7"
                        ...
                        <button class="btn-primary" type="button" id="stockInBtn">
                            <svg class="btn-icon-white" width="20" height="20" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M20 12V20H4V12M20 12L12 4M20 12H13M12 4L4 12M12 4V12" stroke="white"
                        ...
                        <button class="btn-primary" type="button" id="stockOutBtn">
                            <svg class="btn-icon-white" width="20" height="20" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M20 12V20H4V12M20 12L12 20M20 12H13M12 20L4 12M12 20V12" stroke="white"
                                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                                ...
                                <tr class="inventory-row" data-index="{{ $index }}"
                                    ...
                                    <td class="location-cell">
                                        <div class="item-location">
                                            <svg class="location-icon" width="16" height="16"
                                                ...
                                                <path
                                                    ...
                                                    stroke="#235C63" stroke-width="2" stroke-linecap="round"
                                                    stroke-linejoin="round" />
                                                <path
                                                    d="M15 11C15 12.657 13.657 14 12 14C10.343 14 9 12.657 9 11C9 9.343 10.343 8 12 8C13.657 8 15 9.343 15 11Z"
                                                    stroke="#235C63" stroke-width="2" stroke-linecap="round"
                                                    stroke-linejoin="round" />
                                            </svg>
                                    ...
                                    <td class="actions-cell">
                                        <button class="action-link transfer-trigger-btn" type="button"
                                            ...
                                            data-rack="{{ $item->Rack ?? '' }}" data-area="{{ $item->Area ?? '' }}">
                                            Update Location
                                        </button>
                                        <button class="action-link delete-inventory-btn"
                                                type="button"
                                                data-inventory-id="{{ $item->Inventory_ID }}"
                                                data-product-name="{{ $item->product->Product_Name ?? 'Unknown' }}"
                                                style="color: #ef4444; background: none; border: none; cursor: pointer; font-size: 13px; font-weight: 500; padding: 4px 8px; border-radius: 6px; transition: background 0.2s;"
                                                title="Delete Inventory Record">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 4px;">
                ...
                <div class="inventory-alerts">
                    @if (isset($lowStock) && $lowStock->count() > 0)
                        <div class="alert alert-warning">
                            <div class="alert-content">
                                <svg class="alert-icon" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M12 9V11M12 15H12.01M5.07183 19H18.9282C19.7423 19 20.2105 18.0645 19.7303 17.4087L12.8021 7.94723C12.4239 7.43075 11.5761 7.43075 11.1979 7.94723L4.26973 17.4087C3.78954 18.0645 4.25773 19 5.07183 19Z"
                                        stroke="#D97706" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                                <div class="alert-text">
                                    <span class="alert-title">Low Stock Alert</span>
                                    <span class="alert-description">{{ $lowStock->count() }} items below
                                        threshold</span>
                                </div>
                            </div>
                        </div>
                    ...
                    @php
                        $outOfStockCount = isset($inventoryItems)
                            ? $inventoryItems
                        ...
                        <div class="alert alert-danger">
                            <div class="alert-content">
                                ...
                                <div class="alert-text">
                                    ...
                                    <span class="alert-description">{{ $outOfStockCount }} items need
                                        restocking</span>
                                </div>
                            </div>
                        </div>
                    ...
                    @forelse($transfers as $transfer)
                        <div class="transfer-item" data-id="{{ $transfer->Transaction_ID }}">
                            <div class="transfer-info">
                                ...
                                <div class="transfer-route">{{ $transfer->sourceLocation->Location_Name ?? '?' }}
                                    &rarr; {{ $transfer->destinationLocation->Location_Name ?? '?' }}</div>
                            </div>
...
<script>
    ...
    `;
        ...
        `;
            ...
            modal.querySelector('#submitStockIn').addEventListener('click', async function() {
                await submitFormModal(modal, '#stockInForm',
                    '{{ route('admin.inventory.stock-in') }}', 'Stock in recorded!');
            });
        });
        ...
        `;
            ...
            modal.querySelector('#submitStockOut').addEventListener('click', async function() {
                await submitFormModal(modal, '#stockOutForm',
                    '{{ route('admin.inventory.stock-out') }}', 'Stock out recorded!');
            });
        });
        ...
        `;
            ...
            modal.querySelector('#submitTransfer').addEventListener('click', async function() {
                if (!confirm(
                        'Are you sure you want to proceed with this stock transfer? This will immediately update inventory levels in both locations.'
                        )) {
                    return;
                }
                await submitFormModal(modal, '#transferForm',
        ...
        document.querySelectorAll('.transfer-trigger-btn').forEach(btn => {
            ...
            `;
                ...
                modal.querySelector('#submitUpdateLocation').addEventListener('click',
                async function() {
                        await submitFormModal(modal, '#updateLocationForm',
                            '{{ route('admin.inventory.update-location') }}',
                            'Location updated!');
                    });
            });
        });
        ...
        document.getElementById('openAgingReportBtn')?.addEventListener('click', function() {
            ...
            if (!itemsHTML) {
                itemsHTML =
                    '<div style="text-align:center;padding:2rem;color:#6b7280;">No aging stock found.</div>';
            }
        ...
        `;
            createModal('Aging Inventory Report', body,
        ...
        `;
            ...
            const loadLogs = async (date) => {
                ...
                listContainer.innerHTML =
                    '<div style="text-align:center;padding:2rem;color:#64748b;">Loading scans...</div>';
                ...
                try {
                    ...
                    `).join('');
                        ...
                        listContainer.innerHTML =
                            '<div style="text-align:center;padding:2rem;color:#64748b;">No scan logs found for this date.</div>';
                    }
                } catch (err) {
                    listContainer.innerHTML =
                        '<div style="text-align:center;padding:2rem;color:#dc2626;">Error loading logs.</div>';
                }
            };
        ...
        window.deleteLogEntry = async function(id) {
            if (!confirm(
                    'Are you sure you want to delete this scan entry? This will reverse the stock impact.'
                    )) {
                return;
            }
    ...
    document.addEventListener('click', function(e) {
        const btn = e.target.closest('.delete-inventory-btn');
        if (!btn) return;
        ...
        const body = `
            <p>Are you sure you want to delete the inventory record for <strong>${productName}</strong>?</p>
            <p style="margin-top: 10px; font-size: 0.9em; color: #6b7280;">
                This will soft-delete the record — it will be hidden from the inventory table
                but can be recovered from the database if needed. This action will NOT
                affect the product catalog or any historical transaction logs.
            </p>
        `;

        const footer = `
            <button class="inv-btn secondary inv-close-modal">Cancel</button>
            <button class="inv-btn primary" id="confirmDeleteBtn" style="background: #ef4444; border-color: #ef4444;">Confirm Delete</button>
        `;

        const modal = createModal('Delete Inventory Record', body, footer);
        ...
        modal.querySelector('#confirmDeleteBtn').addEventListener('click', async function() {
            ...
            try {
                const response = await fetch('/admin/inventory/' + inventoryId, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json'
                ...
                if (response.ok && result.success) {
                    showToast(result.message || 'Inventory record deleted.', 'success');
                    const row = document.querySelector('.inventory-row[data-id="' + inventoryId + '"]');
                    if (row) {
    ...
    // ========== FORM SUBMISSION HELPER ==========
        ...
        function applyInventoryFilters() {
            ...
            rows.forEach(row => {
                ...
                const matchesSearch = !searchTerm || itemName.includes(searchTerm) || productCode
                    .includes(searchTerm);
...
<style>
    ...
    .inv-modal-overlay.show .inv-modal-content {
